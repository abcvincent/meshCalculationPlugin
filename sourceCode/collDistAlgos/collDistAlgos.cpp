...
// What is following? Can't remember! (26/7/2011)
/* DISABLED FOR NOW (ENABLE AGAIN ONCE POLYGONS WORK!)

        if (int(collNode->leafTriangles->size())>CIloIlo::userSettings->triCountInOBB*64) // too small number (e.g. *1) and it becomes really slow (2x)
        {
            ...
            C3Vector s;
            C4X4Matrix transf(CCollNode::divideElements(s,geom->calcVertices,geom->calcIndices,*collNode->leafTriangles,geom->calcSegments,*collNode->leafSegments,
                tIndexLeft,sIndexLeft,tIndexRight,sIndexRight));
            collNode->leftCollNode=new CCollNode(geom->calcVertices,geom->calcIndices,tIndexLeft,geom->calcSegments,sIndexLeft,655351); // no subdivisions wanted!
            collNode->rightCollNode=new CCollNode(geom->calcVertices,geom->calcIndices,tIndexRight,geom->calcSegments,sIndexRight,655351); // no subdivisions wanted!
            delete collNode->leafTriangles;
            collNode->leafTriangles=NULL;
            delete collNode->leafSegments;
            collNode->leafSegments=NULL;

            //collNode->leafTriangles->clear();
            //collNode->leafSegments->clear();


            //collNode->leftCollNode->size=collNode->size;
            //collNode->rightCollNode->size=collNode->size;
            //collNode->leftCollNode->transformMatrix=collNode->transformMatrix;
            //collNode->rightCollNode->transformMatrix=collNode->transformMatrix;
        }
*/
#endif
    }
...
{   // Only referenced vertices are taken into account (or all if indices is NULL)
    ...
    if (firstPass)
    {
        ...
        {
            C4Vector savedV(v);
            C4Vector rot(piValTimes2/float(divNb[i]),C3Vector::unitZVector);
...
//*/
#endif // MARC_ONLY_COMPILATION_STUFF
